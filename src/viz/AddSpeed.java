...
public class AddSpeed extends DensiTree {
		...
		if (node.isRoot()) {
			...
			if (node.isLeaf()) {
				if (node.getParent().getMetaData() != null) {
					float fX = g_densitree.m_settings.m_fLongitude.get(node.m_iLabel);//m_nCurrentPosition[node.m_iLabel]%32;
					float fY = g_densitree.m_settings.m_fLatitude.get(node.m_iLabel);//m_nCurrentPosition[node.m_iLabel]/32;
	...
	public static void main(String[] args) {
		...
		try {
			g_densitree.m_settings.m_sLabels = new Vector<String>();
			g_densitree.m_settings.m_fLongitude = new Vector<Float>();
			g_densitree.m_settings.m_fLatitude = new Vector<Float>();
			...
			for (int i = 0; i < g_densitree.m_settings.m_sLabels.size(); i++) {
				buf.append("\t\t" + i + " " + g_densitree.m_settings.m_sLabels.get(i));
				if (i < g_densitree.m_settings.m_sLabels.size()-1) {
					buf.append(",");
				}
				buf.append("\n");
			}
