...
*/		} else {
	...
	float m_fAlpha;
//
//	public void draw(int i, float[][] fLinesX, float[][] fLinesY, float [][] fLineWidthX, float [][] fTopLineWidth, Graphics2D g, float fScaleX, float fScaleY, boolean bVariableWidth) {
//		float fAlpha = ((AlphaComposite)g.getComposite()).getAlpha();
//		m_fAlpha = fAlpha;
//		float fLineWidth = ((BasicStroke)g.getStroke()).getLineWidth();
//		Color color = g.getColor();
...
//			} else {
//				if (m_bRootAtTop) {
//					drawBlockTree(fLinesX[i], fLinesY[i], fLineWidthX[i], fTopLineWidth[i], g, fScaleX, fScaleY);
//				} else {
//					drawBlockTree(fLinesY[i], fLinesX[i], fLineWidthX[i], fTopLineWidth[i], g, fScaleX, fScaleY);
//				}
//			}
//		} else {
//			if (fLineWidthX == null) {
