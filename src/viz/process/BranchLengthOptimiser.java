...
public class BranchLengthOptimiser {
	...
	public void optimiseScore(Node tree) {
		...
		float [] heights = new float[m_dt.m_settings.m_sLabels.size() * 2 - 1];
		Node [] nodes = new Node[m_dt.m_settings.m_sLabels.size() * 2 - 1];
...
//		}
		...
		if (m_dt.m_sOptFile != null) {
			try {
				...
				out.println(tree.toString(m_dt.m_settings.m_sLabels, false));
				out.close();
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void initialiseTree(float[] heights, Node[] nodes) {
		...
		for (int k = m_dt.m_settings.m_sLabels.size(); k < nodes.length; k++) {
			Node node = nodes[k];
